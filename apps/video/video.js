...
var LiveRoom = function () {
    ...
    live_socket.onopen = function(){
        // 登录
        var login_data = '{"type":"init","room_id":"' + room_id + '","id":"' + uid + '", "username":"' + uname + '", "avatar":"' + avatar + '", "sign":"' + sign + '"}';
        live_socket.send( login_data );
        console.log("websocket握手成功2!");
    };
    ...
    var live_userOnline=function(data)
    {
        ...
        console.log(data);
        var time = new Date();
        var message = preparePost('in', (time.getHours() + ':' + time.getMinutes()), data.username, data.avatar, '已上线');
        message = $(message);
        chatUserContainer.append(message);
        chatVipUserContainer.append(message);
        live_slim();
    ...
    var live_logMessage=function(data)
    {
        if(data.type=='group' && data.id==room_id)
        {
            ...
            {
                var time = new Date();
                var message = preparePost('out', (time.getHours() + ':' + time.getMinutes()), data.username, data.avatar, data.content);
            }else{
                var time = new Date();
                var message = preparePost('in', (time.getHours() + ':' + time.getMinutes()), data.username, data.avatar, data.content);
            }
